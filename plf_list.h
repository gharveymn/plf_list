...
// Compiler-specific defines used by list:
	...
	#define PLF_LIST_FORCE_INLINE __forceinline
	...
	#endif
	#if _MSC_VER >= 1800
		#define PLF_LIST_DEFAULT_FUNCTIONS
		...
		#define PLF_LIST_CPP17_CONSTEXPR constexpr
	#else
...
#  define PLF_LIST_CPP20_CONSTEXPR constexpr
#else
#  define PLF_LIST_CPP20_CONSTEXPR
#endif

#if __cpp_constexpr >= 201304L
#  define PLF_LIST_CPP14_CONSTEXPR constexpr
#else
#  define PLF_LIST_CPP14_CONSTEXPR
#endif
...
private:
	...
	struct node_base
	{
		...
		#ifdef PLF_LIST_MOVE_SEMANTICS_SUPPORT
			PLF_LIST_CONSTEXPR node_base(node_pointer_type &&n, node_pointer_type &&p) PLF_LIST_NOEXCEPT:
				next(std::move(n)),
				previous(std::move(p))
			{}
		#endif
	...
	struct node : public node_base
	{
		...
		{}
		...
		#ifdef PLF_LIST_MOVE_SEMANTICS_SUPPORT
			PLF_LIST_CONSTEXPR node(node_pointer_type &&next, node_pointer_type &&previous, element_type &&source) PLF_LIST_NOEXCEPT:
			...
			PLF_LIST_CONSTEXPR node(node_pointer_type const next, node_pointer_type const previous, arguments&&... parameters):
				node_base(next, previous),
	...
	struct group : public node_allocator_type // Node memory block + metadata
	{
		...
		{}
			...
			PLF_LIST_CONSTEXPR group(const group_size_type group_size, node_pointer_type const previous = NULL):
		...
		#ifdef PLF_LIST_MOVE_SEMANTICS_SUPPORT
			PLF_LIST_CPP14_CONSTEXPR group(group &&source) PLF_LIST_NOEXCEPT:
				node_allocator_type(source),
	...
	public:
		...
		struct ebco_pair2 : allocator_type // empty-base-class optimisation
		{
			...
			PLF_LIST_CONSTEXPR explicit ebco_pair2(const size_type number_of_elements) PLF_LIST_NOEXCEPT: capacity(number_of_elements) {};
		...
		#ifdef PLF_LIST_TYPE_TRAITS_SUPPORT
			template <typename G = group_pointer_type,
			  typename std::enable_if<std::is_trivial<G>::value>::type * = nullptr>
		...
		#ifdef PLF_LIST_MOVE_SEMANTICS_SUPPORT
			PLF_LIST_CPP14_CONSTEXPR group_vector(group_vector &&source) PLF_LIST_NOEXCEPT:
		...
		#ifdef PLF_LIST_TYPE_TRAITS_SUPPORT
		template <typename T = group_pointer_type, typename std::enable_if<std::is_trivial<T>::value>::type * = nullptr>
      ...
      PLF_LIST_CPP20_CONSTEXPR ~group_vector() = default;
		...
		#ifdef PLF_LIST_TYPE_TRAITS_SUPPORT
		template <typename T = element_type, typename U = node_pointer_type,
			typename std::enable_if<std::is_trivially_destructible<T>::value && std::is_trivially_destructible<U>::value>::type * = nullptr>
		PLF_LIST_CPP14_CONSTEXPR void destroy_all_data(const node_pointer_type&) PLF_LIST_NOEXCEPT
		{
		...
		template <typename T = element_type, typename U = node_pointer_type,
			typename std::enable_if<std::is_trivially_destructible<T>::value && std::is_trivially_destructible<U>::value>::type * = nullptr>
		PLF_LIST_CPP14_CONSTEXPR void clear(const node_pointer_type&) PLF_LIST_NOEXCEPT
		{
		...
		template <typename T = element_type, typename U = node_pointer_type,
			typename std::enable_if<!std::is_trivially_destructible<T>::value && !std::is_trivially_destructible<U>::type>::type * = nullptr>
		#endif
		PLF_LIST_CPP14_CONSTEXPR void clear(const node_pointer_type last_endpoint_node) PLF_LIST_NOEXCEPT
		{
			for (group_pointer_type current_group = block_pointer; current_group != last_endpoint_group; ++current_group)
		...
		template <typename T = node_pointer_type,
		  typename std::enable_if<std::is_trivially_copyable<T>::value &&
		                          std::is_trivially_destructible<T>::value>::type * = nullptr>
		...
		template <typename T = node_pointer_type,
			typename std::enable_if<!(std::is_trivially_copyable<T>::value && std::is_trivially_destructible<T>::value) &&
															std::is_move_constructible<T>::value>::type * = nullptr>
		PLF_LIST_CPP14_CONSTEXPR void expand_capacity(const size_type new_capacity) // used by add_new and append
		{
			group_pointer_type const old_block = block_pointer;
			block_pointer = PLF_LIST_ALLOCATE(group_allocator_type, group_allocator_pair, new_capacity, 0);
		...
		template <typename T = group_pointer_type,
		  typename std::enable_if<std::is_trivial<T>::value>::type * = nullptr>
		...
		PLF_LIST_CPP14_CONSTEXPR void trim_unused_groups() PLF_LIST_NOEXCEPT // trim trailing groups previously allocated by reserve() or retained via erase()
		{
...
public:
	...
	public:
		...
		PLF_LIST_CPP14_CONSTEXPR inline typename plf_enable_if_c<!IsConst, list_iterator &>::type operator = (const list_iterator<IsConst> &rh) PLF_LIST_NOEXCEPT
		{
			node_pointer = rh.node_pointer;
			return *this;
		}
		...
		template <bool IsConst>
		PLF_LIST_CONSTEXPR list_iterator(const list_iterator<IsConst> &source, typename plf_enable_if_c<!IsConst, int>::type = 0) PLF_LIST_NOEXCEPT: node_pointer(source.node_pointer) {}
	...
	public:
		...
		template <bool IsConst>
		PLF_LIST_CPP14_CONSTEXPR inline typename plf_enable_if_c<!IsConst, list_reverse_iterator &>::type operator = (const list_reverse_iterator<IsConst> &rh) PLF_LIST_NOEXCEPT
		{
			node_pointer = rh.node_pointer;
			return *this;
		}
		...
		template <bool IsConst>
		PLF_LIST_CONSTEXPR list_reverse_iterator(const list_reverse_iterator<IsConst> &source, typename plf_enable_if_c<!IsConst, int>::type = 0) PLF_LIST_NOEXCEPT: node_pointer(source.node_pointer) {}
...
private:
	...
	struct ebco_pair1 : node_pointer_allocator_type // Packaging the group allocator with least-used member variables, for empty-base-class optimisation
	{
		...
		PLF_LIST_CONSTEXPR explicit ebco_pair1(const size_type total_num_elements) PLF_LIST_NOEXCEPT: total_number_of_elements(total_num_elements) {}
	}		node_pointer_allocator_pair;

	struct ebco_pair2 : node_allocator_type
	{
		size_type number_of_erased_nodes;
		PLF_LIST_CONSTEXPR explicit ebco_pair2(const size_type num_erased_nodes) PLF_LIST_NOEXCEPT: number_of_erased_nodes(num_erased_nodes) {}
	}		node_allocator_pair;
...
public:
	...
	PLF_LIST_CPP14_CONSTEXPR explicit list(const size_type fill_number, const element_type &element, const element_allocator_type &alloc = element_allocator_type()):
		element_allocator_type(alloc),
		end_node(reinterpret_cast<node_pointer_type>(&end_node), reinterpret_cast<node_pointer_type>(&end_node)),
		last_endpoint(NULL),
		node_pointer_allocator_pair(0),
	...
	{
		reserve(fill_number);
		insert(end(), fill_number, element);
	}
	...
	PLF_LIST_CPP14_CONSTEXPR explicit list(const size_type fill_number, const element_allocator_type &alloc = element_allocator_type()):
		element_allocator_type(alloc),
		end_node(reinterpret_cast<node_pointer_type>(&end_node), reinterpret_cast<node_pointer_type>(&end_node)),
		last_endpoint(NULL),
		node_pointer_allocator_pair(0),
	...
	{
		reserve(fill_number);
		insert(end(), fill_number, element_type());
	}
	...
	PLF_LIST_CPP14_CONSTEXPR list(const typename plf_enable_if_c<!std::numeric_limits<iterator_type>::is_integer, iterator_type>::type &first, const iterator_type &last, const element_allocator_type &alloc = element_allocator_type()):
		element_allocator_type(alloc),
		end_node(reinterpret_cast<node_pointer_type>(&end_node), reinterpret_cast<node_pointer_type>(&end_node)),
		last_endpoint(NULL),
		node_pointer_allocator_pair(0),
		node_allocator_pair(0)
	{
		insert<iterator_type>(end(), first, last);
	}
	...
	#ifdef PLF_LIST_INITIALIZER_LIST_SUPPORT
	PLF_LIST_CPP14_CONSTEXPR list(const std::initializer_list<element_type> &element_list, const element_allocator_type &alloc = element_allocator_type()):
			element_allocator_type(alloc),
			end_node(reinterpret_cast<node_pointer_type>(&end_node), reinterpret_cast<node_pointer_type>(&end_node)),
			last_endpoint(NULL),
			node_pointer_allocator_pair(0),
	...
	{
		return reinterpret_cast<node_pointer_type>(&end_node);
	}
	...
	{
		return reinterpret_cast<node_pointer_type>(const_cast<node_base *> (&end_node));
	}
...
private:
	...
	inline void insert_initialize()
	{
		...
		node_pointer_allocator_pair.total_number_of_elements = 1;
			...
			construct_node(Args&&... args)
			{
				PLF_LIST_CONSTRUCT(node_allocator_type, node_allocator_pair, last_endpoint++,
													 end().node_pointer, end().node_pointer, std::forward<Args> (args)...);
			}
			...
			construct_node(Args&&... args)
			{
				try
				{
					PLF_LIST_CONSTRUCT(node_allocator_type, node_allocator_pair, last_endpoint++, end().node_pointer,
														 end().node_pointer, std::forward<Args> (args)...);
				}
				catch (...)
				{
					reset();
		...
		// copies
			...
			PLF_LIST_CPP14_CONSTEXPR typename std::enable_if<std::is_nothrow_copy_constructible<Node>::value>::type
			construct_node(const element_type &element) PLF_LIST_NOEXCEPT
			{
				#ifdef PLF_LIST_VARIADICS_SUPPORT
					PLF_LIST_CONSTRUCT(node_allocator_type, node_allocator_pair, last_endpoint++, end().node_pointer, end().node_pointer, element);
				#else
					PLF_LIST_CONSTRUCT(node_allocator_type, node_allocator_pair, last_endpoint++, node(end().node_pointer, end().node_pointer, element));
				#endif
			};
		...
		construct_node(const element_type &element) PLF_LIST_NOEXCEPT
		{
			...
			#ifdef PLF_LIST_TYPE_TRAITS_SUPPORT
				template <typename Node = node>
				PLF_LIST_CPP14_CONSTEXPR typename std::enable_if<std::is_nothrow_move_constructible<Node>::value>::type
...
public:
	...
	PLF_LIST_CPP14_CONSTEXPR iterator insert(const const_iterator it, const element_type &element)
	{
		if (last_endpoint != NULL) // ie. list is not empty
		{
			...
			{
				group_pointer_type const node_group = groups.get_nearest_freelist_group((it != end()) ? it.node_pointer : end_node.previous);
	...
	PLF_LIST_CPP14_CONSTEXPR inline PLF_LIST_FORCE_INLINE void push_back(const element_type &element)
	{
		insert(end(), element);
	}
	...
	#ifdef PLF_LIST_MOVE_SEMANTICS_SUPPORT
	  PLF_LIST_CPP14_CONSTEXPR iterator insert(const const_iterator it, element_type &&element) // This is almost identical to the insert implementation above with the only change being std::move of the element
		{
			if (last_endpoint != NULL)
			{
	...
	#ifdef PLF_LIST_VARIADICS_SUPPORT
		template<typename... arguments>
		PLF_LIST_CPP14_CONSTEXPR iterator emplace(const const_iterator it, arguments &&... parameters) // This is almost identical to the insert implementations above with the only changes being std::forward of element parameters, removal of VARIADICS support checking, and is_nothrow_contructible
		{
			if (last_endpoint != NULL)
			{
				if (node_allocator_pair.number_of_erased_nodes == 0)
		...
		template<typename... arguments>
		PLF_LIST_CPP14_CONSTEXPR inline PLF_LIST_FORCE_INLINE reference emplace_back(arguments &&... parameters)
		{
			return (emplace(end(), std::forward<arguments>(parameters)...)).node_pointer->element;
		}
...
public:
			...
			if (number_of_elements > PLF_LIST_BLOCK_MAX)
			{
				...
				if (remainder != 0) // make sure smallest block is first
				{
					...
					{ // Create first group as BLOCK_MIN size then subtract difference between BLOCK_MIN and remainder from next group:
						...
						group_fill_position(element, PLF_LIST_BLOCK_MAX - (PLF_LIST_BLOCK_MIN - remainder), end().node_pointer);
						--multiples;
					}
				}
				else
	...
	template <class iterator_type>
		...
		PLF_LIST_CPP14_CONSTEXPR iterator insert(const const_iterator position, typename plf_enable_if_c<(!std::numeric_limits<iterator_type>::is_integer) && (!std::is_same<typename std::iterator_traits<iterator_type>::iterator_category, std::random_access_iterator_tag>::value), iterator_type>::type first, const iterator_type last)
	#else
	...
	#ifdef PLF_LIST_INITIALIZER_LIST_SUPPORT
		PLF_LIST_CPP14_CONSTEXPR inline iterator insert(const const_iterator it, const std::initializer_list<element_type> &element_list)
		{ // use range insert:
...
public:
	...
	template <typename T = element_type, typename U = node_pointer_type,
	  typename std::enable_if<std::is_trivially_destructible<T>::value &&
	                          std::is_trivially_destructible<U>::value>::type * = nullptr>
	PLF_LIST_CPP14_CONSTEXPR iterator erase(const const_iterator it) // if uninitialized/invalid iterator supplied, function could generate an exception, hence no noexcept
	{
		...
		assert(it.node_pointer != end().node_pointer);
	...
	#ifdef PLF_LIST_MOVE_SEMANTICS_SUPPORT
		// Move assignment
		PLF_LIST_CPP14_CONSTEXPR list & operator = (list &&source) PLF_LIST_NOEXCEPT_MOVE_ASSIGNMENT(allocator_type)
		{
			assert (&source != this);
	...
	PLF_LIST_CPP14_CONSTEXPR bool operator == (const list &rh) const PLF_LIST_NOEXCEPT
	{
		...
		for (const_iterator lh_iterator = begin(), rh_iterator = rh.begin(); lh_iterator != end(); ++lh_iterator, ++rh_iterator)
		{
...
private:
	...
	struct less
	{
		PLF_LIST_CONSTEXPR inline bool operator() (const element_type &a, const element_type &b) const PLF_LIST_NOEXCEPT
		{
			return a < b;
		}
	};
...
public:
	...
	template <class comparison_function, typename T = node_pointer_type,
		        typename std::enable_if<std::is_trivially_destructible<T>::value>::type * = nullptr>
	PLF_LIST_CPP14_CONSTEXPR void sort(comparison_function compare)
	{
	...
	PLF_LIST_CPP14_CONSTEXPR inline PLF_LIST_FORCE_INLINE void trim() PLF_LIST_NOEXCEPT
	{
	...
	#ifdef PLF_LIST_TYPE_TRAITS_SUPPORT
	template <typename T = element_type, typename std::enable_if<std::is_move_assignable<T>::value && std::is_move_constructible<T>::value>::type * = nullptr>
...
public:
	...
	PLF_LIST_CPP14_CONSTEXPR void merge(list &source)
	{
		...
		else
		{
			...
			source.end_node.previous->next = end().node_pointer;
		}
...
private:
	...
	struct eq
	{
		PLF_LIST_CPP14_CONSTEXPR inline bool operator() (const element_type &a, const element_type &b) const PLF_LIST_NOEXCEPT
		{
			return a == b;
		}
	};
...
public:
	...
	template <class comparison_function>
	PLF_LIST_CPP14_CONSTEXPR size_type unique(comparison_function compare)
	{
	...
	PLF_LIST_CPP14_CONSTEXPR size_type remove_if(predicate_function predicate)
	{
  		const size_type original_number_of_elements = node_pointer_allocator_pair.total_number_of_elements;
	...
	PLF_LIST_CPP14_CONSTEXPR void resize(const size_type number_of_elements, const element_type &value = element_type())
	{
		...
		{
			insert(end(), number_of_elements - node_pointer_allocator_pair.total_number_of_elements, value);
		}
	...
	// Range assign:
 	template <class iterator_type>
	PLF_LIST_CPP14_CONSTEXPR inline void assign(const typename plf_enable_if_c<!std::numeric_limits<iterator_type>::is_integer, iterator_type>::type first, const iterator_type last)
	{
		clear();
	...
	// Fill assign:
	PLF_LIST_CPP14_CONSTEXPR inline void assign(const size_type number_of_elements, const element_type &value)
	{
		clear();
		reserve(number_of_elements); // Will return anyway if capacity already > number_of_elements
		insert(end(), number_of_elements, value);
	}
		...
		PLF_LIST_CPP14_CONSTEXPR inline void assign(const std::initializer_list<element_type> &element_list)
		{
			clear();
			reserve(element_list.size());
			insert(end(), element_list);
	...
	PLF_LIST_CPP14_CONSTEXPR iterator unordered_find_single(const element_type &element_to_match) const PLF_LIST_NOEXCEPT
	{
		if (node_pointer_allocator_pair.total_number_of_elements != 0)
		{
			for (group_pointer_type current_group = groups.block_pointer; current_group != groups.last_endpoint_group; ++current_group)
	...
	PLF_LIST_CPP14_CONSTEXPR list<iterator> unordered_find_multiple(const element_type &element_to_match, size_type number_to_find) const
	{
	...
	PLF_LIST_CPP14_CONSTEXPR list<iterator> unordered_find_all(const element_type &element_to_match) const
	{
		list<iterator> return_list;
...
PLF_LIST_CPP14_CONSTEXPR inline void swap(list<swap_element_type, swap_element_allocator_type> &a, list<swap_element_type, swap_element_allocator_type> &b) PLF_LIST_NOEXCEPT_SWAP(swap_element_allocator_type)
{
	a.swap(b);
}
...
#undef PLF_LIST_CPP17_CONSTEXPR
