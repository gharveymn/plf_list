...
#  define PLF_LIST_TIME_TEST
#  include <chrono>
#endif

static std::streambuf& get_null_buffer (void)
{
	static struct null_buffer : std::streambuf
	{
...
static std::ostream out (&get_null_buffer ());
// static std::ostream out (std::cout.rdbuf ());

void title1(const char *title_text)
{
	out << std::endl << std::endl << std::endl << "*** " << title_text << " ***" << std::endl;
...
{
	out << std::endl << std::endl << "--- " << title_text << " ---" << std::endl << std::endl;
}


void title3(const char *title_text)
{
	out << std::endl << title_text << std::endl;
}
...
int main(int argc, char **argv)
{
	...
	using namespace plf;
		...
		std::cout << "Initializer_list support (C++11 or higher) is required for most tests. Most tests will skipped without it." << std::endl;
	#endif

	#ifdef PLF_LIST_TIME_TEST
	...
	for (unsigned int prime_loop_counter = 0; prime_loop_counter != 50; ++prime_loop_counter)
	{
		...
		#ifdef PLF_INITIALIZER_LIST_SUPPORT
		{
			...
			while (!list2.empty())
			{
				...
				{
					out << "Failing at counter == " << test_counter << std::endl;
				}
			}
...

		{
			...
			for (unsigned int loop_counter = 0; loop_counter != 50; ++loop_counter)
			{
				...
				while (!i_list.empty())
				{
					...
					{
						out << "Fail. loop counter: " << loop_counter << ", internal_loop_counter: " << internal_loop_counter << "." << std::endl;
						std::cin.get();
						abort();
					}
